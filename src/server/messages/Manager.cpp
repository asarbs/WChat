...
namespace WChat::ChatServer::messages {
    ...
    void Manager::handle(websocket_server* s, const websocketpp::connection_hdl& hdl, WChat::Msg msg) {
    ...
    void Manager::register_handler(WChat::MessageType message_type_id, std::shared_ptr<handlers::Handler> handler) {
        auto f_handler = _handlers.find(message_type_id);
        if (f_handler == _handlers.end()) {
