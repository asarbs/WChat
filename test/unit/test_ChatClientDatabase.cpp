...
TEST_F(ChatClientDatabaseTest, add_new_clinet_db) {
    ...
    uint32_t user_id = ChatClientDatabase::getInstance().regiserClinetSession(hdl, "asar", "password");
...
TEST_F(ChatClientDatabaseTest, register_10_and_unregiseter_5_clinet) {
    for (uint32_t i = 0; i < 10; i++) {
        websocketpp::connection_hdl hdl;
        ChatClientDatabase::getInstance().regiserClinetSession(hdl, "asar" + std::to_string(i), "password");
    }
...
TEST_F(ChatClientDatabaseTest, create_connection_between_clients) {
    ...
    uint32_t user1 = ChatClientDatabase::getInstance().regiserClinetSession(hdl1, "user1", "password");
    uint32_t user2 = ChatClientDatabase::getInstance().regiserClinetSession(hdl2, "user2", "password");
...
TEST_F(ChatClientDatabaseTest, save_and_check_messages) {
    websocketpp::connection_hdl hdl1, hdl2;
    uint32_t user1 = ChatClientDatabase::getInstance().regiserClinetSession(hdl1, "sender", "password");
    uint32_t user2 = ChatClientDatabase::getInstance().regiserClinetSession(hdl2, "receiver", "password");
