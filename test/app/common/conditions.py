...
async def expect_response_ack(ws):
    ...
    assert response.type                 == messages_pb2.MessageType.RESPONSE
    # assert response.response             == messages_pb2.Response.ACK
    return response
...
async def expect_response_nack(ws1):
    ...
    assert response.version              == 1
    assert response.type                 == messages_pb2.MessageType.RESPONSE
    assert response.response             == messages_pb2.Response.NACK
...
async def wait_for_connection_request(ws_client):
    ...
    assert response.version                     == 1
    assert response.type                        == messages_pb2.MessageType.CONTACT_CONNECTION_REQ
...
async def wait_for_connection(ws_client):
    ...
    response.ParseFromString(raw_data)
    assert response.version                     == 1
    assert response.type                        == messages_pb2.MessageType.CONTACT_CONNECTION_RES
    assert response.contactConnectionRes.status == messages_pb2.Response.ACK
...
async def wait_for_connection_nack(ws_client):
    ...
    assert response.version                     == 1
    assert response.type                        == messages_pb2.MessageType.CONTACT_CONNECTION_RES
    assert response.contactConnectionRes.status == messages_pb2.Response.NACK
    return response.contactConnectionRes.status, response.contactConnectionRes.from_user_id, response.contactConnectionRes.to_user_id
